...
uint16_t UDS::SessionServer(Session_t* session)
{
  struct Message_t msg;
  uint16_t retval;
  ...
  msg.tx_id=session->tx_id;
  msg.Buffer=tmpbuf;
  retval = _isotp->receive(&msg); // check UDS request
  if(!retval)
  {
	session->sid=tmpbuf[0];
	session->Data=tmpbuf+1;
	session->len=msg.len-1;
  }
  return retval;
...
uint16_t UDS::serverResponse(Session_t* session)
{
  struct Message_t msg;
  uint16_t retval;
  ...
  msg.len=session->len+1;
  msg.Buffer=tmpbuf;
  retval = _isotp->send(&msg); // Send response
  return retval;
}
